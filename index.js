...
requestButton$.addEventListener('click', requestPermissions)
notificationButton$.addEventListener('click', showLocalNotification)
...
function main() {
    ...
    navigator.serviceWorker.ready.then((registration) => {
        console.log("[index.js] Service worker is ready!")

        // registration.showNotification("First notification!", {
        //     body: `This is the first notification!`,
        // })

        logOpenedNotifications(registration)

        activateIdleNotifications()
    });
}
...
function requestPermissions() {
    Notification.requestPermission().then((permission) => {
        ...
        if (permission === "denied") {
            alert("[index.js] If you want to send notifications, you must allow notifications in the website's permissions settings.")
        }
    });
}

function logOpenedNotifications(registration) {
...
function showLocalNotification() {
    ...
    navigator.serviceWorker.ready.then(async function (registration) {
        try {
            await registration.showNotification("Notification test " + Math.floor(Math.random() * 100), {
                body: "This is a notification! It should have a smiling face with heart-eyes emoji as an icon and the Daniel Zotti logo as an image! It should display 2 actions: OK and CANCEL.",
                icon: "./images/smiling-face-emoji.png",
                image: "./images/danielzotti-logo.png",
                badge: "./images/danielzotti-logo-small.png",
                ...
                actions: [{
                    ...
                    action: 'cancel', title: 'CANCEL', icon: './images/cancel.png'
                }],
                // silent: false, // must not specify vibrate pattern
                // vibrate: [200, 100, 500, 100, 500, 500, 1000], // silent must be false, doesn't work in Android > 8.0 :(
                // renotify: true, // it requires a tag
                // tag: "reusable-notification",
            })
            console.log("[index.js] Notification sent!");
        } catch (e) {
            console.log(e)
            alert("[index.js] If you want to send notifications, you must allow notifications in the website's permissions settings.")
        }
...
function activateIdleNotifications() {
    // let comeBackNotification; // OLD WAY
        ...
        if (document.visibilityState === "hidden") {
            ...
            navigator.serviceWorker.ready.then(async registration => {
                ...
                registration.addEventListener('click', (e) => {
                    console.log("[index.js] Notification CLICKED!", e.target.data)
                    e.preventDefault();
                    window.parent.focus();
                });
            });
            ...
            // OLD way: "new Notification" will be deprecated in Chrome and it doens't work in Chrome for Android
            ...
            // comeBackNotification.close(); // OLD WAY
            navigator.serviceWorker.ready.then(registration => {
                registration.getNotifications({
                    tag: 'come-back'
                }).then((notifications) => {
                    notifications.forEach(n => {
                        console.log("[index.js] You came back! I can close the 'come back' notification (if any)")
                        n.close();
                    })
                });
            })
